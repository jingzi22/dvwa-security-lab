...
## 操作步骤 (Steps)
1. 登录 DVWA 系统（admin/password），刷新首页捕获已登录状态的 GET /dvwa/index.php 请求
2. 右键该请求，选择 Send to Repeater 发送至重放模块
3. 在 Repeater 左侧 Request 区域，找到 User-Agent 行并修改为 User-Agent: Hello-Hacker
4. 点击 Repeater 左上角 Send 按钮发送修改后请求
5. 查看右侧 Response 区域，验证响应状态码与页面内容是否正常
...
- 服务器返回 200 OK 状态码，证明自定义 User-Agent 可正常被服务端接收
- User-Agent 字段无服务端校验，支持任意自定义内容修改
